...
public class SNEntityListener implements Listener {
			...
			if (fireball.getShooter() instanceof Player) {
				...
				for (Entity entity : fireball.getNearbyEntities(3, 3, 3)) {
					if (entity instanceof LivingEntity) {
						...
						lEntity.damage(SNConfigHandler.demonFireballDamage, ((Player) fireball.getShooter()).getPlayer());
	...
	public void onEntityTarget(EntityTargetEvent event) {
		...
		if (!snplayer.getTruce()) {
			SupernaturalsPlugin.log("No truce " + event.getEntity());
			return;
		}

		if (EntityUtil.entityTypeFromEntity(event.getEntity()) == null) {
			SupernaturalsPlugin.log("No entity v2 " + event.getEntity());
			return;
		}
		...
		if (snplayer.isVampire()
			...
			event.setCancelled(true);
				...
				&& SNConfigHandler.demonTruce.contains(EntityUtil.entityTypeFromEntity(event.getEntity()))) {
			event.setCancelled(true);
		}
		if (event.isCancelled()) {
			SupernaturalsPlugin.log("Event is cancelled "+ event.getEntity());
			return;
		}
		if (!(event.getTarget() instanceof Player)) {
			SupernaturalsPlugin.log("Non-Player target " + event.getEntity());
			return;
		}

		if (event.getEntity() == null) {
			SupernaturalsPlugin.log("No entity");
			return;
		}

		if (!SupernaturalsPlugin.hasPermissions((Player) event.getTarget(), worldPermission)
				&& SNConfigHandler.multiworld) {
			SupernaturalsPlugin.log("Multiworld");
			return;
		}
