		...
		if (event instanceof EntityDamageByEntityEvent) {
			...
			if (victim instanceof Creature) {
				...
				if (snDamager.isVampire()
					...
					plugin.getSuperManager().truceBreak(snDamager);
				} else if (snDamager.isDemon()
						&& SNConfigHandler.demonTruce.contains(EntityUtil.entityTypeFromEntity(cVictim))) {
					plugin.getSuperManager().truceBreak(snDamager);
				}
			}
		}
	...
	public void onEntityDeath(EntityDeathEvent event){
			...
			if (e instanceof EntityDamageByEntityEvent) {
				if (((EntityDamageByEntityEvent) e).getDamager() instanceof LivingEntity) {
					...
					lDamager = (LivingEntity) ((Projectile) ((EntityDamageByEntityEvent) e).getDamager()).getShooter();
				}
			}
