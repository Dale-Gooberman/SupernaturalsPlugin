...
public class DemonManager extends ClassManager {
	...
	public void deathEvent(Player player) {
		...
		if (e.getCause().equals(DamageCause.DROWNING)) {
			...
			Biome pBiome = player.getWorld().getBiome(pLocX,pLocY,pLocZ);
			if (snplayer.isDemon()) {
				if (pBiome == Biome.TAIGA
						...
						|| pBiome == Biome.DEEP_FROZEN_OCEAN
						|| pBiome == Biome.FROZEN_PEAKS
						|| pBiome == Biome.SNOWY_BEACH
						|| pBiome == Biome.SNOWY_PLAINS
						|| pBiome == Biome.SNOWY_TAIGA) {
					if (SNConfigHandler.debugMode) {
	...
	public boolean snare(Player player, Player target) {
		...
		Block block;
		Set<Material> nullSet = new Set<Material>() {
			@Override
			public int size() {
				return 0;
			}
		...
		if (target == null) {
			block = player.getTargetBlock(nullSet, 20);
		} else {
