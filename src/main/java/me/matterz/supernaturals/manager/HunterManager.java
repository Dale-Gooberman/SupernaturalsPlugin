...
public class HunterManager extends HumanManager {
	...
	public boolean doorEvent(Player player, Block block, Door door) {
		...
		if (snplayer.isHunter() || snplayer.isHuman() && open) {
			if (door.isTopHalf()) {
				...
				block.setType(Material.IRON_DOOR,false);//setTypeIdAndData(71, (byte) (block.getData() + 4), false);
				newBlock.setType(Material.IRON_DOOR,false);//setTypeIdAndData(71, (byte) (newBlock.getData() + 4), false);
			} else {
				newLoc = new Location(loc.getWorld(), loc.getBlockX(), loc.getBlockY() + 1, loc.getBlockZ());
				newBlock = newLoc.getBlock();
				block.setType(Material.IRON_DOOR,false);//setTypeIdAndData(71, (byte) (block.getData() + 4), false);
				newBlock.setType(Material.IRON_DOOR,false);//setTypeIdAndData(71, (byte) (block.getData() + 4), false);;
			}
	...
	/*private void closeDoor(Location loc) {
		...
		Block door = (Block) block.getState().getData();
		if (!((Block) block.getState().getData()).isBlockPowered()) {
			return;
		}
		...
		if (door.getState() = ) {
			...
			block.setBlockData(Powerable().setPowered(false),false);//setPowered(false);
			newBlock.setType(Material.IRON_DOOR,false);//setTypeIdAndData(71, (byte) (newBlock.getData() - 4), false);
		} else {
			newLoc = new Location(loc.getWorld(), loc.getBlockX(), loc.getBlockY() + 1, loc.getBlockZ());
			newBlock = newLoc.getBlock();
