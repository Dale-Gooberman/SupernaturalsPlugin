...
public class PriestManager extends HumanManager {
	...
	public void spellEvent(EntityDamageByEntityEvent event, Player target) {
		...
		Material itemMaterial2 = player.getInventory().getItemInOffHand().getType();
		...
		if (itemMaterial2.equals(SNConfigHandler.priestSpellMaterials.get(3))) {
			cancelled = heal(player,player);
		}
