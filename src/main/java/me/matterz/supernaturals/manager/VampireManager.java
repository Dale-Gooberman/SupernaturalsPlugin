...
public class VampireManager extends ClassManager {
	...
	public boolean isUnderRoof(Player player) {
		/*
		 * We start checking opacity 2 blocks up. As Max Y is 320 there CAN be a
		 * roof over the player if he is standing in block 317: 319 Solid Block
		 * 318 317 Player However if he is standing in 319 there is no chance.
		 */
		boolean retVal = false;
		Block blockCurrent = player.getLocation().getBlock();

		if (player.getLocation().getY() >= 319) {
			retVal = false;
		} else {
